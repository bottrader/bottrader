...
class GdaxFeedConnector {
  ...
  onclose() {
    ...
    this.error(new Error('Connection with GDAX has been closed'));
  }
  ...
  connect(pair, streamHandler) {
    return new Promise((resolve, reject) => {
      ...
      const ws = new WebSocket('wss://ws-feed.gdax.com/ws');

      ws.on('open', this.onopen.bind(this));
      ws.on('message', this.onOrderBook.bind(this));
...
// }
  ...
  onOrderBook(data) {
    console.log('Received Depth Packet', data);
...
new GdaxFeedConnector().connect('ETHBTC', (data) => {
  console.log(data);
})
  .then(() => {
